...
run-proxy:
	go run ./cmd/proxy-server/main.go -log-path=${LOGPATH} -grpc-proxy-port=${GRPC_PROXY_PORT} -grpc-server-address=${GRPC_ADRESS}

run-client:
	go run ./cmd/client/main.go -server=localhost:${GRPC_PROXY_PORT}
	# go run ./cmd/client/main.go -server=localhost:${GRPCPORT}

run-client-add-data:
	go run ./cmd/client-add-data/main.go -server=localhost:${GRPCPORT}

build:
	cd ./cmd/server && go build .
	cd ./cmd/proxy-server && go build .